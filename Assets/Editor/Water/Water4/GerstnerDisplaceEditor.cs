...
namespace UnityStandardAssets.Water
{
    ...
    public class GerstnerDisplaceEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var go = ((GerstnerDisplace) serObj.targetObject).gameObject;
            var wb = (WaterBase) go.GetComponent(typeof(WaterBase));
            var sharedWaterMaterial = wb.sharedMaterial;

            GUILayout.Label("Animates vertices using up 4 generated waves", EditorStyles.miniBoldLabel);

            if (sharedWaterMaterial)
            {
                var amplitude = WaterEditorUtility.GetMaterialVector("_GAmplitude", sharedWaterMaterial);
                var frequency = WaterEditorUtility.GetMaterialVector("_GFrequency", sharedWaterMaterial);
                var steepness = WaterEditorUtility.GetMaterialVector("_GSteepness", sharedWaterMaterial);
                var speed = WaterEditorUtility.GetMaterialVector("_GSpeed", sharedWaterMaterial);
                var directionAB = WaterEditorUtility.GetMaterialVector("_GDirectionAB", sharedWaterMaterial);
                var directionCD = WaterEditorUtility.GetMaterialVector("_GDirectionCD", sharedWaterMaterial);
