...
namespace UnityStandardAssets.Water
{
    ...
    public class WaterBaseEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var hasShore = oceanMaterial.HasProperty("_ShoreTex");
            ...
            WaterEditorUtility.SetMaterialColor("_BaseColor",
                EditorGUILayout.ColorField("Refraction",
                    WaterEditorUtility.GetMaterialColor("_BaseColor", oceanMaterial)), oceanMaterial);
            WaterEditorUtility.SetMaterialColor("_ReflectionColor",
                EditorGUILayout.ColorField("Reflection",
                    WaterEditorUtility.GetMaterialColor("_ReflectionColor", oceanMaterial)), oceanMaterial);
            ...
            WaterEditorUtility.SetMaterialTexture("_BumpMap",
                (Texture)
                EditorGUILayout.ObjectField("Normals",
                    WaterEditorUtility.GetMaterialTexture("_BumpMap", waterBase.sharedMaterial), typeof(Texture), false),
                waterBase.sharedMaterial);
            if (hasShore)
                WaterEditorUtility.SetMaterialTexture("_ShoreTex",
                    (Texture)
                    EditorGUILayout.ObjectField("Shore & Foam",
                        WaterEditorUtility.GetMaterialTexture("_ShoreTex", waterBase.sharedMaterial), typeof(Texture),
                        false), waterBase.sharedMaterial);
            ...
            animationTiling = new Vector4(firstTiling.x / 100.0F, firstTiling.y / 100.0F, secondTiling.x / 100.0F,
                secondTiling.y / 100.0F);
            ...
            var displacementParameter = WaterEditorUtility.GetMaterialVector("_DistortParams", oceanMaterial);
            var fade = WaterEditorUtility.GetMaterialVector("_InvFadeParemeter", oceanMaterial);
            ...
            var gerstnerNormalIntensity = WaterEditorUtility.GetMaterialFloat("_GerstnerIntensity", oceanMaterial);
            ...
            if (!oceanMaterial.HasProperty("_Fresnel"))
            {
                ...
                {
                    var fresnelScale = EditorGUILayout.Slider("Intensity",
                        WaterEditorUtility.GetMaterialFloat("_FresnelScale", oceanMaterial), 0.1F, 4.0F);
            ...
            {
                var fresnelTex = (Texture) EditorGUILayout.ObjectField(
                    "Ramp",
                    WaterEditorUtility.GetMaterialTexture("_Fresnel",
                        oceanMaterial),
                    typeof(Texture),
                    false);
            ...
            if (oceanMaterial.HasProperty("_Foam"))
            {
                ...
                var foam = WaterEditorUtility.GetMaterialVector("_Foam", oceanMaterial);
